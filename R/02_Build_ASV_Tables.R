...
sample.names <- fns %>% basename() %>% str_split("_") %>% map_chr(1)
...
saveRDS(dadaFs,"output/ITS_dadaFs.RDS")
...
path <- "./data/raw/StALOHA_18S/cutadapt" 
filtpath <- "./data/clean/SSU" 
if(!file_test("-d", filtpath)) dir.create(filtpath)

fns <- sort(list.files(path, full.names = TRUE, pattern = "_R1_001.fastq.gz$")) 
rns <- sort(list.files(path, full.names = TRUE, pattern = "_R2_001.fastq.gz$")) 

sample.names <- fns %>% basename() %>% str_split("_") %>% map_chr(1)
filts_f <- file.path(filtpath, paste0(sample.names, "_FWD_filt.fastq.gz"))
filts_r <- file.path(filtpath, paste0(sample.names, "_REV_filt.fastq.gz"))
...
## FILTER AND TRIM ####
out <- filterAndTrim(fwd = fns, filt = filts_f, 
                     rev = rns, filt.rev = filts_r, # input and output file names as denoted above
                     maxN=0, # uncalled bases are currently not supported in dada2
...
# save the filtration info in case we need it later
saveRDS(out, "./output/SSU_trackreads.RDS")
...
# learn errors
errF <- learnErrors(filts_f, multithread=TRUE, MAX_CONSIST = 20,verbose = 2) 
saveRDS(errF,"./output/SSU_errF.RDS")
errR <- learnErrors(filts_r, multithread=TRUE, MAX_CONSIST = 20,verbose = 2) 
saveRDS(errF,"./output/SSU_errR.RDS")
...
## REMOVE CHIMERAS ####
seqtab.nochim <- removeBimeraDenovo(seqtab, method="consensus", multithread=TRUE, verbose=TRUE)
saveRDS(seqtab.nochim,"./output/SSU_seqtab.nochim.RDS")
dim(seqtab.nochim)
sum(seqtab.nochim)/sum(seqtab)
...
write.csv(track, file = "./output/SSU_read_counts_at_each_step.csv", row.names = TRUE)
...
# save cleaned up seqtab
saveRDS(seqtab.nochim,"./output/SSU_seqtab.nochim.clean.RDS")


# LSU SEQS ####

path <- "./data/raw/StALOHA_28S/cutadapt" 
filtpath <- "./data/clean/LSU" 
if(!file_test("-d", filtpath)) dir.create(filtpath)
...
# save the filtration info in case we need it later
saveRDS(out, "./output/LSU_trackreads.RDS")
...
# learn errors
errF <- learnErrors(filts_f, multithread=TRUE, MAX_CONSIST = 20,verbose = 2) 
saveRDS(errF,"./output/LSU_errF.RDS")
errR <- learnErrors(filts_r, multithread=TRUE, MAX_CONSIST = 20,verbose = 2) 
saveRDS(errF,"./output/LSU_errR.RDS")
...
## SAMPLE INFERRENCE ####
dadaFs <- dada(derepF, err=errF, multithread=TRUE, selfConsist = TRUE, verbose=TRUE, pool = "pseudo") # set multithread = FALSE on Windows
saveRDS(dadaFs,"output/LSU_dadaFs.RDS")
dadaRs <- dada(derepR, err=errR, multithread=TRUE, selfConsist = TRUE, verbose=TRUE, pool = "pseudo") # set multithread = FALSE on Windows
saveRDS(dadaRs,"output/LSU_dadaRs.RDS")
...
## REMOVE CHIMERAS ####
seqtab.nochim <- removeBimeraDenovo(seqtab, method="consensus", multithread=TRUE, verbose=TRUE)
saveRDS(seqtab.nochim,"./output/LSU_seqtab.nochim.RDS")
dim(seqtab.nochim)
sum(seqtab.nochim)/sum(seqtab)
...
write.csv(track, file = "./output/LSU_read_counts_at_each_step.csv", row.names = TRUE)
...
# save cleaned up seqtab
saveRDS(seqtab.nochim,"./output/LSU_seqtab.nochim.clean.RDS")
